package apps

import (
	"os"
	"time"

	. "github.com/onsi/ginkgo/v2"
	. "github.com/onsi/gomega"
	"github.com/onsi/gomega/gexec"
	"gopkg.in/yaml.v3"

	"csbbrokerpakdatabricks/acceptance-tests/helpers/cf"
	"csbbrokerpakdatabricks/acceptance-tests/helpers/random"
)

const pushWaitTime = 20 * time.Minute

type Option func(*App)

func Push(opts ...Option) *App {
	defaults := []Option{WithName(random.Name(random.WithPrefix("app")))}
	app := App{}
	app.Push(append(defaults, opts...)...)
	return &app
}

func (a *App) Push(opts ...Option) {
	WithOptions(opts...)(a)
	appManifestContainsPath := false
	wdForPush := ""

	cmd := []string{"push"}
	if !a.start {
		cmd = append(cmd, "--no-start")
	}
	if a.buildpack != "" {
		cmd = append(cmd, "-b", a.buildpack)
	}
	if a.memory != "" {
		cmd = append(cmd, "-m", a.memory)
	}
	if a.manifest != "" {
		cmd = append(cmd, "-f", a.manifest)

		var err error
		appManifestContainsPath, err = manifestContainsPath(a.manifest)
		if err != nil {
			Fail("Unable to extract the manifest")
		}
	}

	if a.dir.path() == "" {
		Fail("App directory must be specified")
	}

	if appManifestContainsPath {
		wdForPush = a.dir.path()
	} else {
		cmd = append(cmd, "-p", a.dir.path())
	}
	defer a.dir.cleanup()

	if a.Name == "" {
		Fail("App name must be specified")
	}
	cmd = append(cmd, a.Name)

	var session *gexec.Session
	if wdForPush != "" {
		session = cf.StartWithWorkingDirectory(wdForPush, cmd...)
	} else {
		session = cf.Start(cmd...)
	}
	Eventually(session, pushWaitTime).Should(gexec.Exit())
	checkSuccess(session.ExitCode(), a.Name)

	if session.ExitCode() != 0 {
		GinkgoWriter.Printf("FAILED to push app. Getting logs...")
		cf.Run("logs", a.Name, "--recent")
		Fail("App failed to push")
	}

	a.URL = url(a.Name)
}

func WithBinaryBuildpack() Option {
	return func(a *App) {
		a.buildpack = "binary_buildpack"
		a.memory = "50MB"
	}
}

func WithName(name string) Option {
	return func(a *App) {
		a.Name = name
	}
}

func WithDir(dir string) Option {
	return func(a *App) {
		a.dir = staticDir(dir)
	}
}

func WithManifest(manifest string) Option {
	return func(a *App) {
		a.manifest = manifest
	}
}

func WithStartedState() Option {
	return func(a *App) {
		a.start = true
	}
}

func WithOptions(opts ...Option) Option {
	return func(a *App) {
		for _, o := range opts {
			o(a)
		}
	}
}

func manifestContainsPath(manifest string) (bool, error) {
	manifestBytes, err := os.ReadFile(manifest)
	if err != nil {
		return false, err
	}
	manifestData := struct {
		Applications []struct {
			Path string `yaml:"path"`
		} `yaml:"applications"`
	}{}
	err = yaml.Unmarshal(manifestBytes, &manifestData)
	if err != nil {
		return false, err
	}
	for _, app := range manifestData.Applications {
		if app.Path == "" {
			return false, nil
		}
	}
	return true, nil
}
